...
export function useOfflineSync() {
  ...
  const savePendingActions = useCallback((actions: OfflineAction[]) => {
    localStorage.setItem('bible-daily-pending-actions', JSON.stringify(actions));
    setPendingActions(actions);
  }, []);
  ...
  const syncPendingActions = useCallback(async () => {
    ...
    try {
      ...
      // 마지막 동기화 시간 저장
      localStorage.setItem('bible-daily-last-sync', new Date().toISOString());

      console.log(`✅ 동기화 완료: ${result.successful.length}개 성공, ${result.failed.length}개 실패`);
...
export function useOfflinePost() {
  ...
  const createOfflinePost = useCallback((postData: {
    title: string;
    content: string;
    bibleVerse?: string;
  }) => {
    return addOfflineAction('CREATE_POST', {
      ...postData,
      clientId: `post-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`,
      createdAt: new Date().toISOString(),
    });
  }, [addOfflineAction]);

  const updateOfflinePost = useCallback((postId: string, postData: {
    title?: string;
    content?: string;
    bibleVerse?: string;
  }) => {
  ...
  const deleteOfflinePost = useCallback((postId: string) => {
...
export function useOfflineLike() {
  ...
  const toggleOfflineLike = useCallback((postId: number, isLiked: boolean) => {
...
export function useOfflineMission() {
  ...
  const completeOfflineMission = useCallback((missionId: number, isCompleted: boolean) => {
